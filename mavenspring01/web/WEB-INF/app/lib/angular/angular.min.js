...
(function (x) {
    ...
    function Wc(a, b) {
        ...
        this.$get = ["$injector", "$interpolate", "$exceptionHandler", "$templateRequest", "$parse", "$controller", "$rootScope", "$sce", "$animate", "$$sanitizeUri", function (a,
            ...
            function templateUrlAddLanguage(url){
                var language = typeof window.oaLanguageType === "undefined" ? "CN" : window.oaLanguageType;
                var lastIndex = url.lastIndexOf("/");
                var beforeStr = url.substring(0, lastIndex) + "/";
                var afterStr = url.substring(lastIndex, url.length);
                return beforeStr + language + afterStr;
            }
            ...
            function ga(a, b, d, f, g, h, k, l) {
                var m = [], n, p, K = b[0], r = a.shift(),
                    ...
                    y = D(r.templateUrl) ? r.templateUrl(b, d) : templateUrlAddLanguage(r.templateUrl), E = r.templateNamespace;
                b.empty();
                e(y).then(function (c) {
                    var e, G;
                    c = Ea(c);
                ...
                return function (c, e, m, n, p) {
                    ...
                    c.$watch(f, function (f) {
                        ...
                        if (f && window.oaLanguageType && j) {
                            ...
                            var p2 = Math.max(l.lastIndexOf("/"), l.lastIndexOf("\\"));
                            // console.log(p2+"   p2");
                            var l2 = l.slice(0, p2);
                            var r2 = l.slice(p2);

                            if (k != -1) {
                                f = l2 + "/" + window.oaLanguageType + r2 + f.slice(k);
                            } else {
                                f = l2 + "/" + window.oaLanguageType + r2;
                            }
                            // console.log(f+"   f");
                            // Math.max()
                            // var m2 = l.slice(0, l.lastIndexOf("/"));
                            // var n2 = l.slice(l.lastIndexOf("/"));
                            //
                            // // if(k != -1){
                            // //     f = m2 + "_" + window.oaLanguageType + n2 + f.slice(k);
                            // // }else{
                            // //     f = m2 + "_" + window.oaLanguageType + n2;
                            // // }
                            //
                            // if(k != -1){
                            //     f = m2 + "_" + window.oaLanguageType + n2 + f.slice(k);
